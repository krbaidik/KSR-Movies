...
@section('title2')
    <section class="content-header">
        <h1>
            <a href="{{ route('backend.notices.create') }}" class="btn btn-primary btn-block margin-bottom"><i class="fa fa-pencil-square-o"></i>Create</a>
                            ...
                            <tbody>
                                @forelse($data['rows'] as $i=>$row)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ route('backend.notices.show',$row->id)}}" class="btn btn-warning" type="button"><i class="fa fa-eye" aria-hidden="true"></i></a>
                                            <a href="{{route('backend.notices.edit', $row->id)}}" type="button" class="btn btn-info"><i class="fa fa-edit" aria-hidden="true"></i></a>

                                        {!! Form::open(['method'=>'DELETE', 'url' =>route('backend.notices.destroy', $row->id),'style' => 'display:inline']) !!}
