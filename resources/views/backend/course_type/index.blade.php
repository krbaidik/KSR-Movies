...
@section('title')
    Ksr | course type - list
@endsection

@section('title2')
    <section class="content-header">
        <h1>
            <a href="{{ route('backend.course_type.create') }}" class="btn btn-primary btn-block margin-bottom"><i class="fa fa-pencil-square-o"></i>Create</a>
                            ...
                            <tbody>
                                @forelse($data['rows'] as $i=>$row)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ route('backend.course_type.show',$row->id)}}" class="btn btn-warning" type="button"><i class="fa fa-eye" aria-hidden="true"></i></a>
                                            <a href="{{route('backend.course_type.edit', $row->id)}}" type="button" class="btn btn-info"><i class="fa fa-edit" aria-hidden="true"></i></a>

                                        {!! Form::open(['method'=>'DELETE', 'url' =>route('backend.course_type.destroy', $row->id),'style' => 'display:inline']) !!}
