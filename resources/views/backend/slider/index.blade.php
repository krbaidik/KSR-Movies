...
@section('title2')
    <section class="content-header">
        <h1>
            <a href="{{ route('backend.sliders.create') }}" class="btn btn-primary btn-block margin-bottom"><i class="fa fa-pencil-square-o"></i>Create</a>
                            ...
                            <tbody>
                                @forelse($data['rows'] as $i=>$row)
                                    <tr>
                                        <td>{{ $i+1 }}</td>
                                        <td>{{ $row->title }}</td>
                                        <td><img src="{{ asset('images/sliders/'.$row->slider_image) }}" alt="{{ $row->slider_image }}" width="200" class="img img-thumbnail"></td>
                                        ...
                                        <td>
                                            <a href="{{ route('backend.sliders.show',$row->id)}}" class="btn btn-warning" type="button"><i class="fa fa-eye" aria-hidden="true"></i></a>
                                            <a href="{{route('backend.sliders.edit', $row->id)}}" type="button" class="btn btn-info"><i class="fa fa-edit" aria-hidden="true"></i></a>

                                        {!! Form::open(['method'=>'DELETE', 'url' =>route('backend.sliders.destroy', $row->id),'style' => 'display:inline']) !!}
